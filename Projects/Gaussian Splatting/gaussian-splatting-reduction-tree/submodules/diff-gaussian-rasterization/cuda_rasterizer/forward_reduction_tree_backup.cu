...
__device__ int g_print_once = 0;
__device__ unsigned g_mismatch = 0;
    ...
    float* __restrict__ invdepth)
{
    ...
    for (uint32_t base = range.x; base < range.y; base += 32)
    {
        ...
        // After you compute stop_mask and (optionally) neutralize/recompute prefix_prod:

        uint32_t chunk_last = 0;

        // If we hit early stop in this chunk, the last valid contributor is stop_lane-1,
        // but since contributor is 1-based in the original code (contributor++ before checks),
        // the last_contributor becomes contributor_base + stop_lane.
        if (stop_mask) {
            const int stop_lane = __ffs(stop_mask) - 1;
            // stop_lane is within [0, chunk_len-1]
            chunk_last = contributor_base + (uint32_t)stop_lane;
        } 
        else {
            // No stop: last contributor is the last lane that actually contributed (alpha >= 1/255)
            unsigned contrib_mask = __ballot_sync(mask, (alpha >= (1.0f / 255.0f)));
            if (contrib_mask) {
                const int last_lane = 31 - __clz(contrib_mask);
                if (last_lane < chunk_len)
                    chunk_last = contributor_base + (uint32_t)last_lane + 1u; // normal 1-based
            }
        }        

        //unsigned contrib_mask = __ballot_sync(mask, (alpha >= (1.0f/255.0f)) && (T_after_lane >= T_EPS));
        //uint32_t chunk_last = 0;
        //if (contrib_mask)
        //{
            //const int last_lane = 31 - __clz(contrib_mask);
        ...
        if (stop_mask && lane == 0) {
            const int stop_lane = __ffs(stop_mask) - 1;
            const uint32_t expected_last = contributor_base + (uint32_t)stop_lane;
            if (chunk_last != expected_last) {
                if (atomicInc(&g_mismatch, 0xffffffffu) < 10) {
                    printf("[STOP_MISMATCH] pix=(%d,%d) base=%u stop_lane=%d chunk_last=%u expected=%u chunk_len=%d\n",
                           pix_x, pix_y, base, stop_lane, chunk_last, expected_last, chunk_len);
                }
            }
        }
        ...
        last_contributor = __shfl_sync(mask, last_contributor, 0);
        ...
        //if (lane == 0 && pix_x == 528 && pix_y == 0 && chunk_len < 32) {
            //printf("[TAIL] base=%u chunk_len=%d mask=0x%08x\n", base, chunk_len, mask);
        //}
    ...
    if (lane == 0)
    {
        final_T[pix_id] = T;
        if (lane == 0) {
            const uint32_t range_len = range.y - range.x;
            if (last_contributor > range_len) {
                printf("[BAD_LAST] pix=(%d,%d) last=%u range_len=%u range=(%u,%u) T=%e\n",
                       pix_x, pix_y, last_contributor, range_len, range.x, range.y, T);
            }
        }
        ...
        if (pix_x == 528 && pix_y == 0) {
            printf("[FINAL_T] %.10e\n", final_T[pix_id]);
            printf("[N_CONTRIB] %u\n", n_contrib[pix_id]);
        }

        // ---- DEBUG CHECK ----
        if (pix_x == 528 && pix_y == 0 && (atomicCAS(&g_print_once, 0, 1) == 0))
        {
            printf("[FWD_CHECK] pix=(%d,%d) T=%e last=%u C0=%e C1=%e C2=%e\n",
                   pix_x, pix_y,
                   T,
                   last_contributor,
                   C[0],
