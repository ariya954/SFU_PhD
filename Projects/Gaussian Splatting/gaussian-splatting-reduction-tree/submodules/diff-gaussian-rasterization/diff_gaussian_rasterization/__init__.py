...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer, invdepths = _C.rasterize_gaussians(*args)
            ...
            #bad = (~torch.isfinite(color)).nonzero(as_tuple=False)
            #print("[PY NAN] color has NaN/Inf. first bad idx:", bad[0].tolist(),
                  #"min/max/mean:", float(color.min()), float(color.max()), float(color.mean()))
            #raise RuntimeError("Forward produced NaN/Inf in color")
    ...
    def backward(ctx, grad_out_color, _, grad_out_depth):
        ...
        # Return grads in the SAME ORDER as forward() inputs:
        return (
            ...
            None,                  # raster_settings (non-tensor)
        )
