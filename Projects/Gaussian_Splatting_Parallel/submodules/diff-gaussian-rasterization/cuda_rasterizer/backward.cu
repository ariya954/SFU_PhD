...
// BACKWARD renderCUDA with reduction-tree accumulation to cut atomics
template <uint32_t C>
    ...
    float* __restrict__ dL_dinvdepths)
{
    ...
    const uint32_t tid = (uint32_t)block.thread_rank();
    ...
    // per-j reduction buffers (per-thread partials)
    __shared__ float4 sh_col_inv[BLOCK_SIZE]; // (dcol0,dcol1,dcol2,dinv)
    __shared__ float4 sh_con_op[BLOCK_SIZE];  // (dconx,dcony,dconw,dop)
    __shared__ float2 sh_mean2[BLOCK_SIZE];   // (dmx,dmy)

    const float T_final = inside ? final_Ts[pix_id] : 0.0f;
    float T = T_final;

    uint32_t contributor = (uint32_t)toDo;
    ...
    // ---- DEBUG: print toDo / n_contrib for ONE pixel once ----
    //if (inside && pix.x == 528 && pix.y == 0 && tid == 0) {
        //printf("[BCHECK] pix=(%u,%u) pix_id=%u range=(%u,%u) toDo=%d n_contrib=%d T_final=%e rounds=%d\n",
               //pix.x, pix.y, pix_id, range.x, range.y, toDo, last_contributor, T_final, rounds);
    //}
    ...
    if (inside)
    {
        ...
        if (dL_invdepths) dL_invdepth = dL_invdepths[pix_id];
    }
    ...
    {
        block.sync();
        const int progress = i * BLOCK_SIZE + (int)tid;
        if ((int)range.x + progress < (int)range.y)
        {
            const int coll_id = (int)point_list[range.y - progress - 1];
            collected_id[tid] = coll_id;
            collected_xy[tid] = points_xy_image[coll_id];
            collected_conic_opacity[tid] = conic_opacity[coll_id];
            #pragma unroll
            for (uint32_t ch = 0; ch < C; ch++)
                collected_colors[ch * BLOCK_SIZE + tid] = colors[coll_id * C + ch];
            if (dL_invdepths)
                collected_depths[tid] = depths[coll_id];
        }
        block.sync();

        const int jmax = min(BLOCK_SIZE, toDo);
        for (int j = 0; !done && j < jmax; j++)
        {
            // reset partials
            sh_col_inv[tid] = make_float4(0, 0, 0, 0);
            sh_con_op[tid]  = make_float4(0, 0, 0, 0);
            sh_mean2[tid]   = make_float2(0, 0);

            // which gaussian in the sorted list are we on (per-pixel)
            contributor--;
            bool valid = ((int)contributor < last_contributor);
            ...
            if (valid)
            {
                ...
                if (valid)
                {
                    ...
                    if (valid)
                    {
                        ...
                        float dcol0 = 0.0f, dcol1 = 0.0f, dcol2 = 0.0f;
                        ...
                        for (uint32_t ch = 0; ch < C; ch++)
                        {
                            ...
                            const float v = dchannel_dcolor * dL_dchannel;
                            if constexpr (C == 3)
                            {
                                if (ch == 0) dcol0 = v;
                                if (ch == 1) dcol1 = v;
                            ...
                            {
                                atomicAdd(&(dL_dcolors[global_id * C + ch]), v);
                            }
                        }
                        ...
                        if (dL_dinvdepths)
                        {
                            ...
                            dinv = dchannel_dcolor * dL_invdepth;
                        }
                        ...
                        const float dop = G * dL_dalpha;

                        if constexpr (C == 3)
                            sh_col_inv[tid] = make_float4(dcol0, dcol1, dcol2, dinv);
                        else
                            sh_col_inv[tid] = make_float4(0, 0, 0, dinv);

                        sh_con_op[tid] = make_float4(dconx, dcony, dconw, dop);
                        sh_mean2[tid]  = make_float2(dmx, dmy);
                    }
                }
            }
            ...
            // reduction tree (BLOCK_SIZE must be power-of-two)
            for (int stride = BLOCK_SIZE / 2; stride > 0; stride >>= 1)
            {
                if ((int)tid < stride)
                {
                    float4 a = sh_col_inv[tid], b = sh_col_inv[tid + stride];
                    sh_col_inv[tid] = make_float4(a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w);

                    a = sh_con_op[tid]; b = sh_con_op[tid + stride];
                    sh_con_op[tid] = make_float4(a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w);

                    float2 c2 = sh_mean2[tid], d2 = sh_mean2[tid + stride];
                    sh_mean2[tid] = make_float2(c2.x + d2.x, c2.y + d2.y);
                }
                block.sync();
            }
            ...
            if (tid == 0)
            {
                const float4 S1 = sh_con_op[0];
                const float2 S2 = sh_mean2[0];
                const float4 S3 = sh_col_inv[0];

                if (S1.x != 0.f || S1.y != 0.f || S1.z != 0.f || S1.w != 0.f ||
                    S2.x != 0.f || S2.y != 0.f ||
                    S3.x != 0.f || S3.y != 0.f || S3.z != 0.f || S3.w != 0.f)
                {
                    const int global_id = collected_id[j];

                    atomicAdd(&dL_dmean2D[global_id].x, S2.x);
                    atomicAdd(&dL_dmean2D[global_id].y, S2.y);

                    atomicAdd(&dL_dconic2D[global_id].x, S1.x);
                    atomicAdd(&dL_dconic2D[global_id].y, S1.y);
                    atomicAdd(&dL_dconic2D[global_id].w, S1.z);

                    atomicAdd(&dL_dopacity[global_id], S1.w);

                    if constexpr (C == 3)
                    {
                        atomicAdd(&dL_dcolors[global_id * C + 0], S3.x);
                        atomicAdd(&dL_dcolors[global_id * C + 1], S3.y);
                        atomicAdd(&dL_dcolors[global_id * C + 2], S3.z);
                    }

                    if (dL_dinvdepths)
                        atomicAdd(&dL_dinvdepths[global_id], S3.w);
                }
            }
