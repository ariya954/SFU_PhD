...
__forceinline__ __device__ float warpInclusiveProd(float v, unsigned mask)
{
    const int lane = threadIdx.x & 31;

    // If I'm not participating, return neutral (won't be used by lane0 anyway)
    if ((mask & (1u << lane)) == 0) return 1.0f;
    ...
    for (int offset = 1; offset < 32; offset <<= 1)
    {
        ...
        // Only multiply if that src lane is actually in-mask
        if (src >= 0 && (mask & (1u << src)))
            v *= n;
    }
    return v;
}

__forceinline__ __device__ float warpReduceSum(float v, unsigned mask)
{
    const int lane = threadIdx.x & 31;

    if ((mask & (1u << lane)) == 0) v = 0.0f;

    for (int offset = 16; offset > 0; offset >>= 1)
        v += __shfl_down_sync(mask, v, offset);

    return v; // lane0 gets full sum
}
