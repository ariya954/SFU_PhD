...
class CLIP:
    ...
    # @cached_property
    @classmethod
    def _init_clip_model(self, device: torch.device):
        from nerfbridge.features.clip import load

        print(f"Loaded CLIP model {CLIPArgs.model_name}")
        self.model, self.preprocess = load(CLIPArgs.model_name, device=device)
    ...
    def extract_clip_features(
        ...
        if self.model is None or self.preprocess is None:
            CLIP._init_clip_model(device=device)

        # # Patch the preprocess if we want to skip center crop
        # if CLIPArgs.skip_center_crop and self:
        #     # Check there is exactly one center crop transform
        #     is_center_crop = [
        #         isinstance(t, CenterCrop) for t in preprocess.transforms
        #     ]
        #     assert (
        #         sum(is_center_crop) == 1
        #     ), "There should be exactly one CenterCrop transform"
        ...
        if image_paths is not None:
            ...
            images = [rgb_image_transform(image) for image in images]
